...
namespace rtx
{
    void Scene::registerCone(const libconfig::Setting &cone)
    {
        Point3 origin;
        double radius = 0;
        double height = 0;
        ...
        if (cone.exists("normalmap")) {
            ...
            hittable = Factory::createHittable<Cone>(origin, radius, height, _materialMap[matName], std::make_shared<ImageTexture>(path));
        } else {
            hittable = Factory::createHittable<Cone>(origin, radius, height, _materialMap[matName]);
        }
        ...
        const double coneHalfHeight = height / 2.0;

        Point3 minPoint(origin.x() - radius, origin.y() - coneHalfHeight, origin.z() - radius);
        Point3 maxPoint(origin.x() + radius, origin.y() + coneHalfHeight, origin.z() + radius);
