...
namespace rtx
{
    void Scene::registerUnlimitedCylinder(const libconfig::Setting &cylinder)
    {
        Point3 origin;
        double radius = 0;
        std::string mat;
        ...
        if (!cylinder.exists("center") || !cylinder.exists("radius"))
            return;
        ...
        if (cylinder.exists("normalmap")) {
            ...
            hittable = Factory::createHittable<UnlimitedCylinder>(origin, radius, _materialMap[matName], std::make_shared<ImageTexture>(path));
        } else {
            hittable = Factory::createHittable<UnlimitedCylinder>(origin, radius, _materialMap[matName]);
        }
        ...
        Point3 minPoint(origin.x() - radius, origin.y() - radius, origin.z() / 2);
        Point3 maxPoint(origin.x() + radius, origin.y() + radius, origin.z() / 2);
